...
import FestiveMenuCard from "./FestiveMenuCard";
...
const FestiveMenus = () => {
  ...
  const fixmenuCompCollectionRef = collection(db, "festivemenuComponent");
  ...
  const editFixmenuCompData = async (fmd) => {
    const aboutDoc = doc(db, "festivemenuComponent", fmdId);
